#! /bin/bash

[[ -n ${1+x} ]] && build=$1
[[ -z ${build+x} ]] && build=$(date +"%Y%m%d")

#--- Download all the genomes from FTP
umask 0002
mkdir -p NCBI_rep_protozoa/${build}/Archive && cd NCBI_rep_protozoa/${build}/Archive

# First parse the directory listing to get available species
curl -s ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/protozoa/*/ | tr -s ' ' | cut -f9 -d' ' > SPECIES.txt

# Now download the representative genome DNA file for each
cat SPECIES.txt | while read sp; do
    echo $sp
    rsync --partial --progress -iav ftp.ncbi.nlm.nih.gov::genomes/refseq/protozoa/${sp}/representative/*/*_genomic.fna.gz .
done

rm -f *_rna_* && rm -f *_cds_* #removing the rna and cda files

rsync --partial --progress -iav ftp.ncbi.nlm.nih.gov::genomes/refseq/protozoa/assembly_summary.txt .

cut -f 1,8,7 assembly_summary.txt | sed 's/ /_/g'| grep -v '^#' > mapping.txt

#--- Update the sequence IDs with taxonomy ID
for p in *_genomic.fna.gz; do
    fn=$(basename $p)
    bd=${fn%_*}
    acc=$(echo $bd | cut -d "_" -f1,2)
    ti=$(grep "$acc" mapping.txt | cut -f2)
    gunzip -c $p | perl -lne 's/^>([\S]+)/>ti|'$ti'|ref|\1|/; print $_;' > ti_${fn%.*}
done

#--- Create fasta with all sequences
cd ..
cat Archive/ti_*.fna > ref_protozoa_rep_genomes.fna

#--- Create .gitignore
echo -e '*\n!.gitignore' > .gitignore

#--- Build the index with bowtie2
sbatch -N 1 -t 300 -p short,defq  <<EOF
#! /bin/bash
umask 0002
module load bowtie2/2.2.3
bowtie2-build ref_protozoa_rep_genomes.fna ref_protozoa_rep_genomes
EOF
